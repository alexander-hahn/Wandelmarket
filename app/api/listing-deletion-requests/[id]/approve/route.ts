...
export async function POST(
  ...
  const requestRows = await prisma.$queryRaw<
    Array<{ id: string; itemId: string; status: string }>
  >`
  ...
  if (requestRow.status !== "pending") {
    return NextResponse.json({ error: "Only pending deletion requests can be approved" }, { status: 400 });
  }
  ...
  await prisma.$executeRaw`
    ...
    SET "status" = ${"deleted"}, "convertedItemId" = ${null}, "updatedAt" = ${now}
    WHERE "convertedItemId" = ${requestRow.itemId}
  `;

  await prisma.$executeRaw`
    DELETE FROM "ShopItem"
    WHERE "id" = ${requestRow.itemId}
  `;

  await prisma.$executeRaw`
    UPDATE "ListingDeletionRequest"
    SET "status" = ${"approved"}, "reviewedByUserId" = ${auth.user.id}, "updatedAt" = ${now}
    WHERE "id" = ${id}
  `;
